...
with tab_goes:
    ...
    if not diag["disponible"]:
        st.info(
            "El análisis GOES no está disponible. La exposición por ráfagas y códigos "
            "meteorológicos de las otras pestañas sigue funcionando sin él.",
            icon="🛰️",
        )
        if diag["carpeta"] and diag["error"]:
            st.error(
                f"La carpeta **está** en `{diag['carpeta']}`, pero el import falla:\n\n"
                f"`{diag['error']}`\n\n"
                "Suele ser una dependencia que falta o un archivo que no se copió completo."
            )
        elif diag["nombres_parecidos"]:
            st.error(
                "No encontré `granizo_riesgo`, pero sí una carpeta con nombre parecido: "
                + ", ".join(f"`{n}`" for n in diag["nombres_parecidos"])
                + ". En Linux el nombre distingue mayúsculas y no admite espacios: "
                "tiene que llamarse exactamente `granizo_riesgo`."
            )
        else:
            st.caption(
                "Para habilitarlo, copiá la carpeta `granizo_riesgo/` **en la raíz** del "
                "repositorio, al lado de `app.py` — no dentro de `agrolens/`. Tiene que "
                "incluir `__init__.py` y los demás `.py`."
            )
